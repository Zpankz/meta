...
KNOWN_CLUSTER_PREFIXES = tuple(f"{i:02d}_" for i in range(0, 100))
def _now_iso() -> str:
...
def dump_all(base: Path) -> dict:
    ...
    issue_log = {
        "generated_at": _now_iso(),
        "node_count": len(canonical_nodes),
        "cluster_count": len([cluster for cluster in clusters if clusters[cluster]]),
        "hard_edges": len(hard_edges),
        "soft_edges": len(soft_edges),
        "hard_edges_dag": len(hard_edges_dag),
        "acyclic_ratio_raw": raw_scc["acyclic_ratio"],
        ...
        "soft_bridge_candidates": len(candidates),
        "max_depth": max_depth,
    }
    _dump_json(base / ".index" / "issue_log.json", issue_log)
